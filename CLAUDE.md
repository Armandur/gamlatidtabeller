...
4. `app/services/realtime.py` pollar GTFS-RT (TripUpdates +
   ServiceAlerts + VehiclePositions) BEHOVSSTYRT. **RT-kvoten är
   30 000 anrop/rullande 30 dagar (Bronze)** - kontinuerlig pollning
   brände hela kvoten på två dygn (2026-07-07). Nu: pollning sker bara
   när någon använt appen senaste 90 s (`mark_activity()` från
   sidor/API), TripUpdates var 30 s, ServiceAlerts var 5 min,
   VehiclePositions bara när någon har kartan öppen (?karta=1).
   Vid 429 pausas pollningen 30 min. Anropsräknare syns i admin.
